...
def send_msg_all(data_parse):
    msg_bytes = bytes(data_parse, encoding="utf-8")
    # 接收的第一字节，一般都是x81不变
    first_byte = b"\x81"
    ...
    for conn in sock_pool:
        try:
            if conn._closed:
                sock_pool.remove(conn)
            else:
                conn.sendall(msg)
...
def handler_accept(sock):
    while True:
        ...
        threading.Thread(target=handler_msg, args=(conn,)).start()
...
def server_socket():
    ...
    sock.listen(5)
    # t = threading.Thread(target=handler_accept(sock)).start()
    threading.Thread(target=handler_accept, args=(sock,)).start()
        ...
        for message in chat:
            try:
                ...
                if author.get("images") is not None:
                    images = author.get("images")[0].get("url")
                emotes_count = 0
                ...
                if len(sock_pool) > 0:
                    try:
                        msg_data = [name, messages, images, emotes_count]
                        send_msg_all(json.dumps(msg_data))
                    except Exception as e:
                        print(e)
            except Exception as e:
                print(e)
